<div align="center">
  <h2>DoVideoAI</h2>
  <p align="center">
    <a href="https://github.com/jjj-n/DoVideoAI/stargazers"><img src="https://img.shields.io/github/stars/jjj-n/DoVideoAI?style=flat-square" alt="GitHub Stars"></a>
...
- **异步削峰** — RocketMQ 将视频解析移出请求线程，提交后立即返回 `202 Accepted`；系统不生成独立 taskId，前端以 `mediaId + goal + mode` 作为逻辑任务身份查询状态和订阅 SSE。
- **提交去重 + 消费互斥** — 请求入口按「内容指纹 + 模式感知的目标摘要」通过 Redis `setIfAbsent` 原子写入带 6 小时 TTL 的 activeKey，阻止相同任务重复提交；MQ 消费端再使用 Redisson `RLock`，防止重投或多实例消费者在同一时刻重复执行。
- **成本护栏** — Redisson `RRateLimiter` 分别执行用户级 5 次/分钟和全局 30 次/分钟的令牌桶限流；ASR 与模型调用采用有限次数的指数退避重试，兜底第三方网络抖动。
...
如果这个项目对你有帮助，欢迎点一个 Star ⭐ 支持一下，也欢迎通过 Issue 交流。
